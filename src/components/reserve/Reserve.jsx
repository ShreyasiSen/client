...
const Reserve = ({ setOpen, hotelId }) => {
  ...
  return (
    <div className="reserve fixed bottom-10 left-60 ml-32 bg-slate-50 pl-20 pr-40 py-8 shadow-slate-700 shadow-2xl">
      <div className="rContainer ">
        <FontAwesomeIcon
          icon={faCircleXmark}
          className="rClose relative text-lg left-64 ml-28 cursor-pointer"
          onClick={() => setOpen(false)}
        />
        <span className="font-bold text-xl flex justify-start mb-10 ">Select your rooms:</span>
        {data.map((item) => (
        ...
        <button onClick={handleClick} className="rButton  bg-blue-500 rounded-md px-8 py-2 text-white font-bold ml-24">
          Reserve Now!
        </button>
      </div>
    </div>
